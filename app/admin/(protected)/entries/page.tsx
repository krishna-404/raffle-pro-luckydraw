...
import { getEntries, type Entry, type PaginationParams } from "./actions";
...
export default function EntriesPage() {
  ...
  const fetchEntries = async (params: PaginationParams) => {
    try {
      setLoading(true);
      const { data, total, page, pageSize, totalPages } = await getEntries(params);
      setEntries(data);
      setTotal(total);
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={10} className="text-center">No entries found</TableCell>
                </TableRow>
              ) : (
                entries.map((entry) => (
