...
import { getWinners, type PaginationParams, type Winner } from "./actions";
...
export default function WinnersPage() {
  ...
  const fetchWinners = async (params: PaginationParams) => {
    try {
      setLoading(true);
      const { data, total, page, pageSize, totalPages } = await getWinners(params);
      setWinners(data);
      setTotal(total);
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={9} className="text-center">No winners found</TableCell>
                </TableRow>
              ) : (
                winners.map((winner) => (
                  <TableRow key={winner.id}>
                    ...
                    <TableCell>{format(new Date(winner.created_at), 'PPp')}</TableCell>
                    <TableCell>
                      <Button
                        variant="ghost"
                        size="icon"
