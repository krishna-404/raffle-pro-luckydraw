...
import { CalendarPlus, Eye, Gift, Trash2 } from "lucide-react";
	...
	const handleFindWinners = async (eventId: string) => {
		try {
			...
			if (result.error) {
				...
				setWinnerSuccess(
					`Successfully selected ${result.winnersCount} winners for the event!`,
				);
				fetchEvents(); // Refresh the events list to update winner counts
			}
		} catch (error) {
			setWinnerError(
				error instanceof Error ? error.message : "Failed to find winners",
			);
		} finally {
			setFindingWinners(false);
	...
	return (
		<DashboardShell>
			...
			<div className="space-y-4">
							...
							{loading ? (
								...
								events.map((event) => (
									<TableRow key={event.id}>
										...
										<TableCell>
											<span
												className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusBadgeClass(event.status)}`}
											>
												{event.status}
											</span>
										</TableCell>
										...
										<TableCell>
											<div className="flex space-x-2">
												...
												>
													<Gift
														className={`h-4 w-4 ${event.winner_count ? "text-green-600" : "text-muted-foreground"}`}
													/>
												</Button>
												<Button
													variant="ghost"
													size="icon"
													asChild
													title="Preview how this event appears to users"
												>
													<Link href={`/admin/events/preview/${event.id}`}>
				...
				<AlertDialog
					open={!!eventForWinners}
					onOpenChange={() => setEventForWinners(null)}
				>
					<AlertDialogContent>
						<AlertDialogHeader>
							<AlertDialogTitle>Find Winners</AlertDialogTitle>
							<AlertDialogDescription>
								This will randomly select winners for the event based on the
								number of prizes. Once winners are selected, they cannot be
								changed.
							</AlertDialogDescription>
						</AlertDialogHeader>
						<AlertDialogFooter>
							<AlertDialogCancel onClick={() => setEventForWinners(null)}>
								Cancel
							</AlertDialogCancel>
							<AlertDialogAction
								onClick={() =>
									eventForWinners && handleFindWinners(eventForWinners)
								}
				...
				<AlertDialog
					open={showWinners}
					onOpenChange={() => setShowWinners(false)}
				>
					<AlertDialogContent className="max-w-4xl">
						<AlertDialogHeader>
							<AlertDialogTitle>Event Winners</AlertDialogTitle>
							<AlertDialogDescription>
								The following participants have been selected as winners for
								this event.
							</AlertDialogDescription>
						</AlertDialogHeader>
				...
				{winnerError && (
					<AlertDialog
						open={!!winnerError}
						onOpenChange={() => setWinnerError(null)}
					>
						<AlertDialogContent>
							<AlertDialogHeader>
				...
				{winnerSuccess && (
					<AlertDialog
						open={!!winnerSuccess}
						onOpenChange={() => setWinnerSuccess(null)}
					>
						<AlertDialogContent>
							<AlertDialogHeader>
